  ...
  const doExportSVG=()=>{
    ...
    const calloutSvg=callouts.map(c=>{
      ...
      return `<g><line x1="${c.pinX}" y1="${c.pinY}" x2="${c.boxX+bw/2}" y2="${c.boxY+bh}" stroke="${c.borderColor}" stroke-width="1.5" stroke-dasharray="5,3"/>
        ...
        <rect x="${c.boxX}" y="${c.boxY}" width="${bw}" height="${bh}" rx="3" fill="${c.fillColor||c.bgColor}" stroke="${c.borderColor}" stroke-width="1.5"/>
        ${lines.map((line,i)=>`<text x="${c.boxX+12}" y="${c.boxY+20+i*18}" font-size="13" font-family="Arial" font-weight="${i===0?"700":"400"}" fill="${c.textColor||c.borderColor}">${escapeXml(line)}</text>`).join("")}
      </g>`;
    }).join("");
    const curveSvg=curvedLabels.map(cl=>{
      try{
        ...
        return `<defs><path id="${pid}" d="M ${p1.x} ${p1.y} Q ${mx+nx} ${my+ny} ${p2.x} ${p2.y}" /></defs>
          <text font-size="${cl.size}" font-family="Arial" font-weight="700" fill="${cl.color}"><textPath href="#${pid}" startOffset="50%" text-anchor="middle">${escapeXml(cl.text)}</textPath></text>`;
      }catch{return "";}
    }).join("");

    const naSvg=northArrow?`<g transform="translate(20,20)"><circle cx="20" cy="20" r="18" fill="rgba(255,255,255,0.85)" stroke="#111" stroke-width="1.2"/><polygon points="20,2 27,22 20,17 13,22" fill="#111"/><polygon points="20,38 27,18 20,23 13,18" fill="#eee" stroke="#111" stroke-width="0.8"/><text x="20" y="50" text-anchor="middle" font-size="10" font-weight="bold" fill="#111" font-family="Arial">N</text></g>`:"";
