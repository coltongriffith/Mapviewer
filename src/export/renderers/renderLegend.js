...
export function renderLegend(scene) {
  ...
  const rows = items
    .map((item, i) => {
      ...
      return `${swatch}\n    <text x="${labelX}" y="${swatchY + 4}" font-family="Arial" font-size="12" fill="${textColor}">${label}</text>`;
    })
    .join("\n    ");

  return `<g id="legend">
  <rect x="${lx}" y="${ly}" width="${boxWidth}" height="${totalH}" fill="${bg}" stroke="${border}" stroke-width="1" rx="8"/>
  <text x="${lx + padding}" y="${ly + padding + 16}" font-family="Arial" font-size="13" font-weight="bold" fill="${textColor}">Legend</text>
  ${rows}
</g>`;
}
