...
// pointer-events="none" on all text elements prevents them from being
// selectable/highlightable when the SVG is opened in a browser.
...
function renderTitleBlock(scene) {
  ...
  <text x="${tx + 14}" y="${ty + 24}" font-family="Arial" font-size="16" font-weight="bold" fill="#111111" ${PE_NONE}>${title}</text>
  ${subtitle ? `<text x="${tx + 14}" y="${ty + 44}" font-family="Arial" font-size="12" fill="#555555" ${PE_NONE}>${subtitle}</text>` : ""}
</g>`;
}
...
function renderNorthArrow(scene) {
  ...
  <text x="${nx}" y="${ny + 18}" font-family="Arial" font-size="12" font-weight="bold" fill="#111111" text-anchor="middle" ${PE_NONE}>N</text>
  <text x="${nx}" y="${ny + 44}" font-family="Arial" font-size="22" fill="#111111" text-anchor="middle" ${PE_NONE}>↑</text>
</g>`;
}
...
function renderScaleBar(scene) {
  ...
  <text x="${sx + 12}" y="${sy + 36}" font-family="Arial" font-size="11" fill="#111111" ${PE_NONE}>${safeLabel}</text>
</g>`;
}
...
function renderCallouts(scene) {
  ...
  const items = callouts
    ...
    <text x="${c.boxX + 8}" y="${c.boxY + 16}" font-family="Arial" font-size="12" font-weight="600" fill="${borderColor}" ${PE_NONE}>${textLines}</text>
  </g>`;
    })
    .join("\n  ");
